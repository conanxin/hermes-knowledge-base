...
| 审计报告 | hydro0x01 审计、session guard | 属于项目知识库，精选后可迁移为 project |
| 临时文件 | 媒体占位、缓存、日志 | 无价值 |
...
### Batch 3：需要人工判断，精选后可迁移

- 治理报告（skill-catalog 维护报告）— 过时快，但全局总结有价值
- Workshop 笔记 — 个人碎片笔记通常跳过，适配性分析可保留
- 项目报告 — 精选有长期价值的审计报告、设计文档迁移为 project

**标准**：内容可能过时，需要人工判断是否有长期价值；审计报告和设计文档精选后可迁移为 project
